  ...
  class StaticPackageSource < BasicPackageSource
    class DSL
      ...
      def clean_deps(name, version, deps)
        deps.reject {|dep_name, req| name == dep_name && Bundler2::PubGrub::RubyGems.parse_range(req).include?(version) }
      end
    end
    ...
    def parse_dependency(package, dependency)
      ...
      Bundler2::PubGrub::RubyGems.parse_constraint(package, dependency)
    end
  end
end
